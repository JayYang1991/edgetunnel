...
let SOCKS5白名单 = ['*tapecontent.net', '*cloudatacdn.com', '*loadshare.org', '*cdn-centaurus.com', 'scholar.google.com', '*google.com', '*googleapis.com'];
let 反代白名单 = ['*.google.com', "*.googleapis.com", "", "*ipinfo.io"];
const Pages静态页面 = 'https://jayyang1991.github.io/edt-pages.github.io';
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
                        ...
                        if (url.searchParams.get('mode') === 'tunnel') {
                            ...
                            config_JSON.PATH = "/singbox-ws-path";
                            config_JSON.完整节点路径 = "/singbox-ws-path" + (config_JSON.启用0RTT ? '?ed=2560' : '');
                            config_JSON.随机路径 = false;
                        }
                        ...
                        if (订阅类型 === 'mixed') {
                            ...
                            const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 订阅TOKEN + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : '') + (url.searchParams.has('mode') ? `&mode=${url.searchParams.get('mode')}` : ''))}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&scv=${config_JSON.跳过证书验证}`;
                            try {
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID) {
    ...
    const 验证反代白名单 = (addr) => 反代白名单.some(p => new RegExp(`^${p.replace(/\*/g, '.*')}$`, 'i').test(addr));
    if (验证反代白名单(host) || (启用SOCKS5反代 && (启用SOCKS5全局反代 || 验证SOCKS5白名单(host)))) {
...
function Singbox订阅配置文件热补丁(SingBox_原始订阅内容, uuid = null, fingerprint = "chrome", ech_config = null) {
    ...
    try {
        ...
        if (!existingOutboundTags.has('REJECT')) {
            config.outbounds.push({ "type": "block", "tag": "REJECT" });
            existingOutboundTags.add('REJECT');
        }
